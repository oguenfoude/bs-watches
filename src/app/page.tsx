  ...
  if (!mounted) {
    return (
      <div className="flex items-center gap-2 justify-center" dir="ltr">
        {[{ label: "ساعة" }, { label: "دقيقة" }, { label: "ثانية" }].map(({ label }, i) => (
          <div key={label} className="flex items-center gap-2">
            <div className="flex flex-col items-center">
              <span className="bg-gray-900 text-white text-lg font-bold px-3 py-1.5 rounded-lg min-w-[44px] text-center tabular-nums">
                {"00"}
              </span>
              <span className="text-[10px] text-gray-500 mt-1">{label}</span>
            </div>
            {i < 2 && <span className="text-gray-300 font-bold text-lg mb-4">:</span>}
          </div>
        ))}
      </div>
    );
...
export default function Page() {
  ...
  const goTo = useCallback((i: number) => {
    setSlideDir(i > currentSlide ? 1 : -1);
    setCurrentSlide(i);
  }, [currentSlide]);
  ...
  useEffect(() => { setViewers(Math.floor(Math.random() * 8) + 3); }, []);
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-gray-50 text-gray-900" suppressHydrationWarning>
      ...
      {/* ═══════════════════════════════════════════
          MAIN — Two-column: carousel (sticky) | info + form
          ═══════════════════════════════════════════ */}
      <main className="mx-auto max-w-6xl px-4 py-6 md:py-10">
        <div className="grid lg:grid-cols-2 gap-6 lg:gap-10 items-start">

          {/* ── LEFT COL: IMAGE CAROUSEL (sticky on desktop) ── */}
          <div className="space-y-3 lg:sticky lg:top-20">
            ...
            >
              ...
              <button
                ...
                className="absolute right-3 top-1/2 -translate-y-1/2 z-10 w-10 h-10 bg-white/80 hover:bg-white rounded-full flex items-center justify-center shadow-md transition-all hover:scale-110 active:scale-95"
                aria-label="السابق"
              >
              ...
              <button
                ...
                className="absolute left-3 top-1/2 -translate-y-1/2 z-10 w-10 h-10 bg-white/80 hover:bg-white rounded-full flex items-center justify-center shadow-md transition-all hover:scale-110 active:scale-95"
                aria-label="التالي"
              >
            ...
            <div className="flex items-center gap-2 justify-center">
              <button
                onClick={() => { pauseAuto(); prev(); }}
                className="w-8 h-8 rounded-full bg-white hover:bg-gray-100 flex items-center justify-center flex-shrink-0 transition-colors border border-gray-200"
              >
          ...
          <div id="order">
            ...
            >
              ...
              {/* Trust strip */}
              <div className="flex items-center justify-center gap-4 py-2.5 px-4 bg-gray-50 border-b border-gray-100 text-[11px] text-gray-500 font-medium">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                      ...
                      <AnimatePresence>
                        {watch && (
                          <motion.div
                            ...
                            className="mt-2 pt-2 border-t border-gray-200/60"
                          >
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-amber-100 focus:border-amber-400 resize-none bg-white hover:border-gray-300 transition-all"
                      />
                    </div>
