...
// Blocks: all-same digits (0555555555),
//         sequential patterns (0512345678, 0587654321),
//         known test/fake prefixes,
//         numbers that start wrong
// ─────────────────────────────────────────────
function validatePhone(raw: string): string | undefined {
  ...
  // Block all-same digit (e.g. 0555555555, 0666666666)
  if (/^(.)\1+$/.test(d)) return "رقم الهاتف غير صحيح";
  // Block digits 2-10 all same (e.g. 0511111111)
  if (/^0[567](\d)\1{7}$/.test(d)) return "رقم الهاتف غير صحيح";
  // Block obvious sequential (ascending or descending) in last 8 digits
  ...
  // Block first 5 digits repeated (e.g. 0512341234 — less strict but common fake)
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 md:py-10">
        ...
        {/* ═══════════════════════════════════════════
            TWO-COLUMN GRID: image | info+form
            Image goes first on mobile (order-1),
            form goes below (order-2)
          ...
          <div className="order-2 space-y-5" id="order-form">
            ...
            <div>
              ...
              <p className="text-gray-500 mt-2 text-base leading-relaxed">
                {WATCHES.length} موديل حصري — ساعة + طقم إكسسوارات + توصيل لباب بيتك.
              </p>
            </div>
